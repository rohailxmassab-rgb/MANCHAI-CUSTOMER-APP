...
export function MenuProvider({ children }: { children: ReactNode }) {
  ...
  const [featuredCategories, setFeaturedCategories] = useState<Record<string, MenuItemData | null>>({
    Italian: null,
    Chinese: null,
    Asian: null,
    Japanese: null
  });
  const [realTaste, setRealTaste] = useState<{ featured: MenuItemData | null, vegetarian: MenuItemData[], primary: MenuItemData[] }>({
    featured: null,
    vegetarian: [],
    primary: []
  });
  ...
  useEffect(() => {
    ...
    const handleSnapshot = (snapshot: any, source: string) => {
      ...
      snapshot.forEach((doc: any) => {
        ...
        if (s.includes("chinese") || s.includes("chines") || s.includes("china")) fallbackCategory = "Chinese";
        if (s.includes("japanese") || s.includes("japanes")) fallbackCategory = "Japanese";
        if (s.includes("asian")) fallbackCategory = "Asian";

        // Map common field names to ensure compatibility with various admin app schemas
        const resolvedName = data.name || data.title || data.itemName || data.label || data.productName || data.dishname || "";
        
        // Skip system-generated unnamed dishes or placeholders
        if (!resolvedName || resolvedName.toLowerCase().includes("placeholder")) {
          return;
        }

        const rawPrice = data.price ?? data.itemPrice ?? data.cost ?? data.productPrice;
        const formattedPrice = typeof rawPrice === 'number' ? `$${rawPrice.toFixed(2)}` : (rawPrice || "$0.00");

        const mappedItem: MenuItemData = { 
          id: doc.id, 
          name: resolvedName,
          description: data.description || data.desc || data.itemDescription || data.summary || data.productDescription || "",
          price: formattedPrice,
          image: data.image || data.imageUrl || data.imageurl || data.img || data.itemImage || data.photo || data.productImage || "",
          tags: data.tags || data.category || data.itemCategory || data.type || data.productCategory || fallbackCategory,
          bgColor: data.bgColor || "#f4f4f4",
          rating: Number(data.rating || data.itemRating || data.productRating || 5),
          reviews: Number(data.reviews || data.itemReviews || data.productReviews || 0),
          category: data.category || data.itemCategory || data.type || data.productCategory || fallbackCategory,
          timestamp: data.timestamp || data.updatedAt || data.createdAt || data.date || null
        } as any;
    ...
    const updateUnifiedDishes = () => {
      ...
      const sortedItems = [...uniqueSource].sort((a: any, b: any) => {
        ...
        else if (typeof a.timestamp === 'string' || typeof a.timestamp === 'number') timeA = new Date(a.timestamp).getTime();
        
        let timeB = 0;
        if (b.timestamp?.toMillis) timeB = b.timestamp.toMillis();
        else if (b.timestamp?.seconds) timeB = b.timestamp.seconds * 1000;
        else if (typeof b.timestamp === 'string' || typeof b.timestamp === 'number') timeB = new Date(b.timestamp).getTime();

        return timeB - timeA;
      });

      console.log(`Unified Dishes Update: Total unique dishes = ${sortedItems.length}`);
      setDishes(sortedItems);
      setIsLoading(false);
    };

    const unsubscribeDishes = onSnapshot(query(itemsRef), (snapshot) => {
    ...
    const unsubscribeAltItems = onSnapshot(query(altItemsRef), (snapshot) => {
    ...
    const unsubscribeMenuItems = onSnapshot(query(menuItemsRef), (snapshot) => {
    ...
    const unsubscribeItalian = onSnapshot(query(italianRef), (snapshot) => {
    ...
    const unsubscribeChinese = onSnapshot(query(chineseRef), (snapshot) => {
    ...
    const unsubscribeJapanese = onSnapshot(query(japaneseRef), (snapshot) => {
    ...
    const unsubscribeAsian = onSnapshot(query(asianRef), (snapshot) => {
    ...
    const unsubscribeLatest = onSnapshot(query(latestRef), (snapshot) => {
    ...
    const unsubscribeNestedItalian = onSnapshot(query(nestedItalianRef), (snapshot) => {
      allNestedItalianDishes = handleSnapshot(snapshot, "dishes/categories/italian");
      updateUnifiedDishes();
    }, (error) => {
      console.warn("Collection 'dishes/categories/italian' catch:", error);
    });

    const unsubscribeNestedChinese = onSnapshot(query(nestedChineseRef), (snapshot) => {
      allNestedChineseDishes = handleSnapshot(snapshot, "dishes/categories/chinese");
      updateUnifiedDishes();
    }, (error) => {
      console.warn("Collection 'dishes/categories/chinese' catch:", error);
    });

    const unsubscribeNestedChina = onSnapshot(query(nestedChinaRef), (snapshot) => {
      allNestedChinaDishes = handleSnapshot(snapshot, "dishes/categories/china");
      updateUnifiedDishes();
    }, (error) => {
      console.warn("Collection 'dishes/categories/china' catch:", error);
    });

    const unsubscribeNestedChines = onSnapshot(query(nestedChinesRef), (snapshot) => {
      allNestedChinesDishes = handleSnapshot(snapshot, "dishes/categories/chines");
      updateUnifiedDishes();
    }, (error) => {
      console.warn("Collection 'dishes/categories/chines' catch:", error);
    });

    const unsubscribeNestedJapanese = onSnapshot(query(nestedJapaneseRef), (snapshot) => {
      allNestedJapaneseDishes = handleSnapshot(snapshot, "dishes/categories/japanese");
      updateUnifiedDishes();
    }, (error) => {
      console.warn("Collection 'dishes/categories/japanese' catch:", error);
    });

    const unsubscribeNestedJapanes = onSnapshot(query(nestedJapanesRef), (snapshot) => {
      allNestedJapanesDishes = handleSnapshot(snapshot, "dishes/categories/japanes");
      updateUnifiedDishes();
    }, (error) => {
      console.warn("Collection 'dishes/categories/japanes' catch:", error);
    });

    const unsubscribeNestedAsian = onSnapshot(query(nestedAsianRef), (snapshot) => {
      allNestedAsianDishes = handleSnapshot(snapshot, "dishes/categories/asian");
      updateUnifiedDishes();
    }, (error) => {
    ...
    const unsubscribeFeatured = onSnapshot(featuredRef, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        const mappedItem = {
          id: doc.id,
          ...data,
          name: data.name || data.title || data.itemName || data.productName || "",
          description: data.description || data.desc || data.itemDescription || data.summary || `${key} specialties`,
          price: data.price || data.itemPrice || data.cost || data.productPrice || "$0.00",
          image: data.image || data.imageUrl || data.img || data.itemImage || data.photo || "",
          tags: data.tags || data.category || data.itemCategory || data.type || key,
          bgColor: data.bgColor || "#f4f4f4",
        ...
        if (!mappedItem.name || mappedItem.name.toLowerCase().includes("placeholder")) {
          return;
        }
    ...
    const unsubscribeRealTaste = onSnapshot(realTasteRef, (snapshot) => {
    ...
    const processItemData = (doc: any) => {
        const data = doc.data();
        const resolvedName = data.name || data.title || data.dishname || "";
        ...
        return { 
          ...
          ...data,
          name: resolvedName,
          description: data.description || data.desc || data.descriptiom || "",
          price: formattedPrice,
          image: data.image || data.imageUrl || data.imageurl || data.img || "",
    ...
    const updateCombinedRealTaste = () => {
      ...
      setRealTaste(prev => ({ ...prev, vegetarian: uniqueVeg, primary: uniquePrim }));
    };
      ...
      const unsub = onSnapshot(collection(db, path), (snapshot) => {
        ...
        snapshot.forEach((doc) => {
          ...
          if (item) {
             ...
             if (item._cat.includes("veg") || path.toLowerCase().includes("veg") || String(item.name).toLowerCase().includes("veg") || lowerId.includes("veg")) {
               vItems.push(item as any);
             } else if (item._cat.includes("prim") || path.toLowerCase().includes("prim") || String(item.name).toLowerCase().includes("prim") || lowerId.includes("prim")) {
    ...
    const updateCombinedOffers = () => {
      ...
      mergedOffers.forEach(item => {
        const key = String(item.title || item.name || '').toLowerCase().trim();
      ...
      mergedUpcoming.forEach(item => {
        const key = String(item.title || item.name || '').toLowerCase().trim();
        if (key && !uniqueUpcomingMap.has(key)) uniqueUpcomingMap.set(key, item);
      });
    ...
    const processOfferData = (doc: any) => {
      ...
      return {
        ...
        title: resolvedName,
        name: resolvedName,
        subtitle: data.subtitle || data._cat || "",
  ...
  const finalFeatured: Record<string, MenuItemData | null> = { ...featuredCategories };
  ...
  categoriesList.forEach(cat => {
    ...
    const matchedDishes = dishes.filter(d => {
      ...
      const itemTags = Array.isArray(d.tags) ? d.tags.join(" ").toLowerCase() : String(d.tags || "").toLowerCase();
